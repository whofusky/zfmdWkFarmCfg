...
if [[ ! -z "${enInterSsh}" && ${enInterSsh} -eq 1 ]];then
    ...
    do
        new_fw_onerule_op 0 2 "tcp"  "" "${PS2_IP}" "${remoteip}:22"
...
for remoteip in $SCADA_IP $PS1_IP $WS_IP
do
    new_fw_onerule_op 0 2 "tcp"  "" "${PS2_IP}" "${remoteip}:${TIME_PORT}"
done

# Setup Audit Link on Predict Server 2.
new_fw_onerule_op 0 3 "udp" "" "${PS2_IP}:${AUDIT_PORT}" "${AUDIT_IP}"

# Setup for REVERSE ISOLATOR on Predict Server 2.
new_fw_onerule_op 0 1 "all" "${PS2_BID_NIC}" "" ""

# Setup Web on Predict Server 2.
new_fw_onerule_op 0 1 "tcp" "" "${PS2_IP}:${WEB_PORT}" "${WS_IP}"

# Setup Data Center Link on Predict Server 2.
for remoteip in $SCADA_IP $PS1_IP $WS_IP
do
    new_fw_onerule_op 0 1 "tcp" "" "${PS2_IP}:${DC_PORT}" "${remoteip}"
done
new_fw_onerule_op 0 0 "tcp" "" "${PS2_IP}" "${PS1_IP}:${DC_PORT}"

# Setup TCP/IP Servers on Predict Server 2.
PS2_SERVERS=$(convertVLineToSpace "${PS2_SERVERS}")
...
PS2_CLIENTS=$(convertVLineToSpace "${PS2_CLIENTS}")
